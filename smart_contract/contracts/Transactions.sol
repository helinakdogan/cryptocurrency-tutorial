...
contract Transactions {
    ...
    function addToBlockchain(address payable receiver, uint256 amount, string memory message, string memory keyword) public {
        transactionCount += 1;
        transactions.push(TransferStruct( msg.sender, receiver, amount, message, block.timestamp, keyword )); //to store all transactions

        emit Transfer(msg.sender, receiver, amount, message, block.timestamp, keyword); //to transfer the amount
    }
